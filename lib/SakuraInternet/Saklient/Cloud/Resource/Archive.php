 ...
 * @property \SakuraInternet\Saklient\Cloud\Resource\Resource $source アーカイブのコピー元 
...
class Archive extends Resource {
	 ...
	 * 
	 * @access public
	 * @return \SakuraInternet\Saklient\Cloud\Resource\Archive this
	 */
		...
		if ($r != null) {
			...
			if (array_key_exists("SourceDisk", $r)) {
				...
				if ($s != null) {
					$id = $s->{"ID"};
					if ($id != null) {
						$obj = Util::createClassInstance("saklient.cloud.resource.Disk", new \ArrayObject([$this->_client, $s]));
						$this->_source = $obj;
					}
				}
			}
	...
	private function set_sizeMib($v)
	{
		...
		if (!$this->isNew) {
			throw new SaklientException("immutable_field", "Immutable fields cannot be modified after the resource creation: " . "SakuraInternet\\Saklient\\Cloud\\Resource\\Archive#sizeMib");
		}
		$this->m_sizeMib = $v;
		$this->n_sizeMib = true;
		return $this->m_sizeMib;
	...
	public function __set($key, $v) {
		switch ($key) {
			...
			default: throw new SaklientException('non_writable_field', 'Non-writable field: SakuraInternet\\Saklient\\Cloud\\Resource\\Archive#'.$key);
		}
	}
