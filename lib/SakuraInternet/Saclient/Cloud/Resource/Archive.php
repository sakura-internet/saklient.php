 ...
 * @property \SakuraInternet\Saclient\Cloud\Resource\Resource $source アーカイブのコピー元 
...
class Archive extends Resource {
		...
		if ($r != null) {
			...
			if (array_key_exists("SourceDisk", $r)) {
				...
				if ($s != null) {
					$id = $s->{"ID"};
					if ($id != null) {
						$obj = Util::createClassInstance("saclient.cloud.resource.Disk", new \ArrayObject([$this->_client, $s]));
						$this->_source = $obj;
					}
				}
			}
	...
	protected function apiSerializeImpl($withClean=false)
	{
		...
		if ($missing->count() > 0) {
			throw new SaclientException("required_field", "Required fields must be set before the Archive creation: " . implode(", ", (array)($missing)));
		}
		return $ret;
	}
